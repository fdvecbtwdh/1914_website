...
| 卡牌提案 | 统一提案系统（`card_proposals`，type 区分）：**修改提案**（对官方卡提修改方案，批准后数值直接写入正式卡牌）与**转正提案**（玩家自制卡申请转为官方，同卡转 source 不复制数据）；提案复用卡牌编辑器（共享宏），提交后自动在论坛「卡牌修改提案 / 卡牌转正」板块开帖，评论/回复/举报全复用；投票 ±1 可改投可取消，重提沿用原帖保留分数；管理员 批准/不批准/打回（必填备注），批准/不批准自动归档冻结；版本历史快照 + 审核记录（人/时间/前后版本）；卡牌页显示"N 个提案正在讨论中"并可筛选全部历史提案；消息通知打回/批准/未批准/重提 |
| 管理整合 | 后台总览（今日注册/发帖/评论、处罚与安全统计、最近处罚/安全事件/注册用户）；用户详情页（状态/活动/处罚历史/被举报/操作记录，`/admin/users/<id>`）；举报处理页快捷处罚（禁言/临时封禁/永久封禁，原因预填举报原因并联动标记已处理）；论坛帖子管理（搜索/状态筛选/隐藏/恢复/删除）；全站搜索覆盖论坛帖子 |
| 安全防护 | 每秒 30 请求限流（超限 429，持续超限自动封禁 IP 1 小时）；登录失败 5 次锁定 15 分钟；注册限速：15 分钟 5 次尝试、每 IP 每天仅可成功注册 5 个账号（按 Cloudflare 真实客户端 IP 计）；IP 封禁表（自动/手动、到期自动解除、管理员可解禁/手动封禁）；安全事件记录（保留 7 天）；后台"安全防护"页：当前封禁、24 小时防护触发统计、最近事件、手动封禁/解禁；Cloudflare Tunnel 部署读取 CF-Connecting-IP 真实客户端 IP |
| 处罚系统 | 禁言（限制发帖/回复/评论/投稿，可按小时/天/周或永久）与封禁（临时/永久，必填原因）；状态存 users 表、历史存 penalties 表；后端强制检查、到期自动解除；处罚通过站内消息通知用户（含原因与解除时间）；管理员后台一键操作，举报处理页可直达"禁言作者" |
...
├── admin.py           管理后台
├── users.py           用户主页/设置
├── forum.py           社区论坛（帖子列表/发帖/详情，回复复用评论系统；归档冻结）
├── proposals.py       卡牌提案（修改/转正统一流程：编辑器→自动发帖→投票→审核→应用）
├── penalties          处罚历史表（禁言/封禁，见 schema.sql；当前状态存 users 表）
├── ip_bans / security_events  IP 封禁与安全事件表（7 天保留，见 schema.sql）
├── card_proposals     卡牌提案表（type: modification/promotion；版本快照见 card_proposal_versions）

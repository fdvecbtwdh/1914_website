...
| 安全防护 | 每秒 30 请求限流（超限 429，持续超限自动封禁 IP 1 小时）；登录失败 5 次锁定 15 分钟；IP 封禁表（自动/手动、到期自动解除、管理员可解禁/手动封禁）；安全事件记录（保留 7 天）；后台"安全防护"页：当前封禁、24 小时防护触发统计、最近事件、手动封禁/解禁；Cloudflare Tunnel 部署读取 CF-Connecting-IP 真实客户端 IP |
